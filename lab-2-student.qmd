...
ggplot(data = surveys,
       mapping = aes(
         x=weight, 
         y=hindfoot_length,
         alpha = 0.3,  #used website to learn about alpha: https://ggplot2.tidyverse.org/reference/aes_colour_fill_alpha.html#:~:text=Alpha%20refers%20to%20the%20opacity,corresponding%20to%20more%20transparent%20colors.
         color = species)  
       )+
  geom_point()+
  labs(title="Scatterplot of Weight v. Hindfoot Length",
...
ggplot(data = surveys,
  ...
  geom_jitter(alpha=.2, color="pink3")+
  labs(title="Boxplot Comparison of Weight by Species",
       subtitle = "Species",
       x="Weight",
       y=""
  )+
...
Based on the output above, since the p-value is very small we can reject the null at any reasonable significance level. Thus, we can conclude that there is very strong evidence to suggest that at least one rodent species has a different population mean weight than the others.
...
## Challenge 2: Exploring Color Themes
```{r}
#| label: colored-rotated-boxplot

ainsley_palet <- c("#000000", "#E3DAC9", "#318CE7", "#0B486B", "#272941",
                   "#1CCEB7", "#008080", "#1B4D3E", "#2C9C38", "#F0A830",
                   "#FFA4E9", "#E95081", "#7B1E7A", "#841B2D")
  #used this website for the color pallet: https://lospec.com/palette-list/simple-14
...
ggplot(data = surveys,
  ...
  scale_fill_manual(values = ainsley_palet, aesthetics = "fill")+
  labs(title="Boxplot Comparison of Weight by Species",
       subtitle = "Weight",
       y="",
       x="Species"
